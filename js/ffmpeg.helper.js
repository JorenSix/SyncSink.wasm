...
class FFmpegHelper{
	...
	handleFFmpegOutput = (type,message) => {
		...
		if(this.ffmpegLogHandler != null){
			this.ffmpegLogHandler(type,message);
		}
	...
	initialzeFFmpeg = async () => {
		await createFFmpegCore({
		  print:    (line) => { this.handleFFmpegOutput("stdout",line); },
		  printErr: (line) => { this.handleFFmpegOutput("stderr",line); }
		}).then(async (Module) => {
	...
	detectCompletion = (message) => {
		if ((message.includes('kB muxing overhead') || message.includes('Invalid argument') || message.includes('Invalid data found') || message.includes("At least one output file")) && this.runResolve !== null) {
		  ...
		  console.log(this.ffmpegCore);
		  console.log("Detected completion");
		  console.log("exit called completion");
		  this.runResolve();
...
var FFmpegSingleton = (function () {
    ...
    function createInstance() {
        ...
        console.log("Create new helper object")
        return object;
    }
...
module.exports = {  FFmpegHelper: FFmpegHelper, FFmpegSingleton: FFmpegSingleton };
